...
class _MapViewState extends State<MapView> {
  final Completer<GoogleMapController> _gMController = Completer<GoogleMapController>();
  ...
  Future<void> _onMapCreated(controller) async {
    ...
    bool isDarkMode = SchedulerBinding.instance.window.platformBrightness == Brightness.dark; //true if dark
    _upLocation();
  ...
  Future<void> _upLocation() async{
    ...
    try {
      ...
      GMController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(zoom: 17, target: LatLng(double.parse(currentLocation.latitude.toString()), double.parse(currentLocation.longitude.toString())))));
    } on Exception {
